...
fn main() {
    ...
    let scripts_to_run: Vec<ScriptFile> = match init_scripts(opts.scripts) {
        ...
        Err(e) => {
            warning!(
                ...
                opts.greppable || opts.hosts_greppable,
                opts.accessible
            );
            std::process::exit(1);
        }
    ...
    if !opts.greppable && !opts.hosts_greppable && !opts.accessible {
        print_opening(&opts);
    }
    ...
    for socket in scan_result {
        let i = ips.iter().position(|&r| r == socket.ip()).unwrap();
        ports_per_ip
            // .entry(socket.ip())
            .entry(&hosts[i])
    ...
    //     warning!(x, opts.greppable || opts.hosts_greppable, opts.accessible);
    // }
    ...
    for (ip, ports) in &ports_per_ip {
        ...
        if opts.greppable || opts.hosts_greppable || opts.scripts == ScriptsRequired::None {
            println!("{} -> [{}]", &ip, ports_str);
            continue;
        }
        // detail!("Starting Script(s)", opts.greppable || opts.hosts_greppable, opts.accessible);
        ...
        //     match script.run() {
        //         Ok(script_result) => {
        //             detail!(script_result.to_string(), opts.greppable || opts.hosts_greppable, opts.accessible);
        //         }
        //         Err(e) => {
        //             warning!(
...
fn parse_addresses(input: &Opts) -> (Vec<IpAddr>, Vec<String>) {
    ...
    for file_path in unresolved_addresses {
        ...
        if !file_path.is_file() {
            warning!(
                ...
                input.greppable || input.hosts_greppable,
                input.accessible
            );
...
fn parse_address(address: &str, resolver: &Resolver) -> (Vec<IpAddr>,std::string::String) {
    IpCidr::from_str(&address)
        .map(|cidr| (cidr.iter().collect(),address.to_string()))
        .ok()
        .or_else(|| {
            format!("{}:{}", &address, 80)
                .to_socket_addrs()
                .ok()
                .map(|mut iter| (vec![iter.next().unwrap().ip()],address.to_string()))
        })
        .unwrap_or_else(|| resolve_ips_from_host(address, resolver))
}

/// Uses DNS to get the IPS assiocated with host
fn resolve_ips_from_host(source: &str, backup_resolver: &Resolver) -> (Vec<IpAddr>, std::string::String) {
...
) -> Result<(Vec<std::net::IpAddr>, Vec<std::string::String>), std::io::Error> {
...
#[cfg(not(tarpaulin_include))]
/// Parses an input file of IPs and uses those
fn read_hosts_from_file(
    ips: &std::path::Path,
) -> Result<Vec<std::string::String>, std::io::Error> {
    let file = File::open(ips)?;
    let reader = BufReader::new(file);
...
fn infer_batch_size(opts: &Opts, ulimit: RawRlim) -> u16 {
    ...
    if ulimit < batch_size {
        warning!("File limit is lower than default batch size. Consider upping with --ulimit. May cause harm to sensitive servers",
            opts.greppable || opts.hosts_greppable, opts.accessible
        );
        ...
        if ulimit < AVERAGE_BATCH_SIZE {
            ...
            warning!("Your file limit is very small, which negatively impacts RustScan's speed. Use the Docker image, or up the Ulimit with '--ulimit 5000'. ", opts.greppable || opts.hosts_greppable, opts.accessible);
